...
func (e *EncryptionEngineImpl) GenerateDEK() ([]byte, error) {
...
func (e *EncryptionEngineImpl) Encrypt(data string) (string, error) {
	// Generate a new DEK
...
func (e *EncryptionEngineImpl) Decrypt(encryptedData string) (string, error) {
	// Get KEK from key manager
	kek, err := e.keyManager.RetrieveKEK("contextID")
	if err != nil {
		return "", err
...
func (e *EncryptionEngineImpl) DecryptDEK(edek string, kek []byte) ([]byte, error) {
	...
	block, err := aes.NewCipher(kek)
	if err != nil {
		return nil, errors.NewEncryptionError("failed to create cipher for DEK decryption", err)
	}

	// Create GCM mode
	gcm, err := cipher.NewGCM(block)
	if err != nil {
		return nil, errors.NewEncryptionError("failed to create GCM for DEK decryption", err)
	}

	// Extract nonce
	nonceSize := gcm.NonceSize()
	if len(ciphertext) < nonceSize {
		return nil, errors.NewEncryptionError("ciphertext too short for DEK decryption", nil)
	}
	nonce, ciphertext := ciphertext[:nonceSize], ciphertext[nonceSize:]
	...
	if err != nil {
		return nil, errors.NewEncryptionError("failed to decrypt DEK", err)
	}
