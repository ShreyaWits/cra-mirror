...
func TestNewObservabilityStack(t *testing.T) {
	...
	stack := observability.NewObservabilityStack()
...
func TestMockObservabilityStack(t *testing.T) {
	...
	assert.Equal(t, 42.0, mockStack.MockMetrics.HistogramCalls[0].Value)

	assert.Equal(t, 1, len(mockStack.MockLogger.InfoMessages))
	assert.Equal(t, "test info message", mockStack.MockLogger.InfoMessages[0].Args[0])

	assert.Equal(t, 1, len(mockStack.MockLogger.ErrorMessages))
	assert.Equal(t, "test error message", mockStack.MockLogger.ErrorMessages[0].Args[0])
}
