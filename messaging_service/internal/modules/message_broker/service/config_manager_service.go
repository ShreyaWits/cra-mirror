...
func (s *ConfigManagerService) GetFromApiConfiguration() (*config.Config, error) {
	...
	if err != nil {
		data, cacheErr = s.GetDataToCache(context.Background(), constants.SERVICE_NAME)
		if cacheErr != nil {
			return nil, err
		}
		return data, nil
	...
	err = s.SetDataToCache(context.Background(), constants.SERVICE_NAME, data)
	if err != nil {
...
func (s *ConfigManagerService) SetDataToCache(ctx context.Context, key string, cfg *config.Config) error {
	...
	return s.cacheclient.SetCache(ctx, constants.SERVICE_NAME, key, string(data), constants.CACCHE_TTL, uuid.NewString())
}

func (s *ConfigManagerService) GetDataToCache(ctx context.Context, key string) (*config.Config, error) {

	val, found, err := s.cacheclient.GetCache(ctx, constants.SERVICE_NAME, key, uuid.NewString())
